...
function drawPdf(infltext, indexes, doc, table, penyatext, fin, percent, total) {
    ...
    for (let i = 0; i < indexes.length; i++) {
        ...
        let currText = infltext[i];
        let currIndex = indexes[i];
        newTable2T += "<tr>\n" +
            "    <th>" + currText + "</th>\n" +
            "    <th>" + currIndex + "</th>\n" +
            "   </tr>";
...
function addInput() {
    ...
    newdiv.innerHTML = " <input  class=\"main-input input-border col-lg-3\" id=\"col-doc" + currDate + "\" type=\"text\"  placeholder=\"Документ\"\n" +
        ...
        "                    <input class=\"main-input input-border col-lg-3\" id=\"col-dt" + currDate + "\" type=\"number\" placeholder=\"Сплата (грн.)\"\n" +
        "                           data-toggle=\"tooltip\" data-placement=\"top\" data-html=\"true\" title=\"\"\n" +
...
function parsingAllLines() {
    for (let i = 0; i < currDate; i++) {
        let date = new Date(document.getElementById('col-date' + String(i)).value);
...
function calculate() {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        let currTotal = totalCalculate(totalpenya, threePer, totalIndex, colKt);
    ...
    drawPdf(infltext, indexes, doc, table, penyatext, date3, percent, total);
